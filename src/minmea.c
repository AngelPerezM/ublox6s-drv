...
bool minmea_check(const char *sentence, bool strict)
{
    ...
    // Sequence length is limited. LONGITUD DE LA CADENA LEÍDA
    if (strlen(sentence) > MINMEA_MAX_LENGTH + 3)
        return false;

    // A valid sentence starts with "$".
    if (*sentence++ != '$') //SI LA LÍNEA COMIENZA CON $
        return false;

    // The optional checksum is an XOR of all bytes between "$" and "*".
    while (*sentence && *sentence != '*' && isprint((unsigned char) *sentence)) //LEE TODA LA LINEA CARACTER POR CARACTER
        checksum ^= *sentence++;                                                //HASTA LLEGAR AL ASTERISCO
